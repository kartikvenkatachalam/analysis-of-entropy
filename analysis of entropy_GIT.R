...
# following block generates a matrix with cancer-related mutations
...
n1=round(nrow(count_matrix2)*0.33) # determines the split in the dataset, default is top or bottom third
...
#following block determines FDR and identified genes with FDR < 0.05 and information >0, and determines the number of tumors that show mutations in each gene
...
entropy3 <- as.data.frame(entropy3[-c(which(duplicated(entropy3$genes)==T)),])
