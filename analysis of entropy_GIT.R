...
#following block removes genes whose mutations are enriched in tumors that belong to neither the highest nor the lowest expressors
dup <- as.data.frame(which(duplicated(entropy3$genes)==T))
for (dup_rem in 1:nrow(dup)){
  dup_gene <- as.character(entropy3[c(dup_rem),1])
  entropy3 <- as.data.frame(entropy3[-c(which(entropy3$genes==dup_gene)),])
}
