...
disease <- c("BLCA")
gene_examined <- c("ENSG00000090674")
...
consensus <-  read_csv("Dropbox/Kartik/Ongoing projects/Cancer and lysosomal genes/TP53_BLCA project/TCGA data sets/BLCA-2/3. entropy/data/Census_allFri Aug 30 15_11_07 2019.csv") 
...
write.csv(mut_matrix2,"Dropbox/Kartik/Ongoing projects/Cancer and lysosomal genes/TP53_BLCA project/TCGA data sets/BLCA-2/3. entropy/data/MCOLN1/mutation matrix in BLCA.csv")

#open matrix and remove columns (genes) whose sum is zero
count_matrix <- mut_matrix2
remove1 <- which(colSums(count_matrix)==0)
count_matrix2 <- as.data.frame(count_matrix[,-c(remove1)])

#the following list decides split
count_matrix2 <- count_matrix2 %>% arrange(desc(gene_exam_expression))

n1=round(nrow(count_matrix2)*0.33) #adjust as needed
...
#cluster1 - top third in MCOLN1 expression
...
#cluster2 - bottom third in MCOLN1 expression
...
for (x in 1:1000){
  ...
  for (y in 1:nrow(entropy)){
    ...
    b<- t(b)
    test1 <- sum(b>a)/x
    entropy[y,5]=as.double(test1)
  }
}

write_csv(entropy,"Dropbox/Kartik/Ongoing projects/Cancer and lysosomal genes/TP53_BLCA project/TCGA data sets/BLCA-2/3. entropy/data/MCOLN1/entropy-MCOLN1 expression.csv")

names1 <- as.data.frame(entropy$genes)
names1$`entropy$genes` <- paste0(names1$`entropy$genes`,", cluster_1")
names2 <- as.data.frame(entropy$genes)
names2$`entropy$genes` <- paste0(names2$`entropy$genes`,", cluster_2")
names3 <- as.data.frame(rbind(names1,names2))
...
entropy2 <- as.data.frame(cbind(names3,vals3))
entropy2 <- entropy2 %>% mutate(p_adj = p.adjust(entropy2$cluster2_p_value,method = "fdr"))
entropy3 <- filter(entropy2, entropy2$p_adj<0.05)

colnames(entropy3) <- c("genes, cluster","information","nominal_p-val","FDR")
entropy3 <- entropy3 %>% arrange(desc(information))


#determine whether mutations are enriched or avoided
cluster1 <- as.data.frame(count_matrix2[c(1:n1),])
cluster1b <- as.data.frame(count_matrix2[-c(1:n1),])
cluster2 <- as.data.frame(count_matrix2[c((n2-n1+1):n2),])
cluster2b <- as.data.frame(count_matrix2[-c((n2-n1+1):n2),])

entropy3 <- entropy3 %>% mutate(enriched_cluster=0) %>% mutate(remainder_cluster=0)

for (i in 1:nrow(entropy3)){
  gene <- as.data.frame(strsplit(as.character(entropy3[i,1]),", "))
  gene1 <- as.character(gene[1,1])
  clust1 <- as.character(gene[2,1])
  if (clust1 == "cluster_2"){
    value_a <- which(colnames(cluster2)==gene1)
    total_a <- as.data.frame(cluster2[,c(value_a)])
    value_b <- which(colnames(cluster2b)==gene1)
    total_b <- as.data.frame(cluster2b[,c(value_b)])
    entropy3[i,5] <- as.numeric(colSums(total_a))
    entropy3[i,6] <- as.numeric(colSums(total_b))
  } else {
    value_a <- which(colnames(cluster1)==gene1)
    total_a <- as.data.frame(cluster1[,c(value_a)])
    value_b <- which(colnames(cluster1b)==gene1)
    total_b <- as.data.frame(cluster1b[,c(value_b)])
    entropy3[i,5] <- as.numeric(colSums(total_a))
    entropy3[i,6] <- as.numeric(colSums(total_b))
  }
}

entropy3 <- entropy3 %>% mutate(total_mutations=entropy3$enriched_cluster+entropy3$remainder_cluster)
entropy3 <- filter(entropy3,entropy3$information>0)

write_csv(entropy3,"Dropbox/Kartik/Ongoing projects/Cancer and lysosomal genes/TP53_BLCA project/TCGA data sets/BLCA-2/3. entropy/data/MCOLN1/entropy-MCOLN1 expression-sig.csv")

entropy4 <- filter(entropy3,!entropy3$total_mutations<15)

write_csv(entropy4,"Dropbox/Kartik/Ongoing projects/Cancer and lysosomal genes/TP53_BLCA project/TCGA data sets/BLCA-2/3. entropy/data/MCOLN1/entropy-MCOLN1 expression-sig-min 15 mutations.csv")
